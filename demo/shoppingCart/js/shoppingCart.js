...
var shoppingCart =  new Vue({
	...
	methods:{
		//添加新地址
		add:function(){
			this.addAddress.show = true;
		},
		saveAdd:function(){
			this.addAddress.show = false;
			//如果新添加的设为了默认就屏蔽之前的默认地址
			if(this.addAddress.checked){	
				this.addressList.forEach(function(ele){
					ele.checked = false;
				})
			}
		...
		saveEdit:function(){
			...
			/*保存处理*/
			this.addressList[this.editAddress.index].name = this.editAddress.name;
			this.addressList[this.editAddress.index].number = this.editAddress.number;
			this.addressList[this.editAddress.index].address = this.editAddress.Province + this.editAddress.City + this.editAddress.area  + this.editAddress.street;
		},
		//撤销编辑添加操作
		undo:function(){
			this.editAddress.show = false;
		...
		chooseProvincesEdit:function(){
			var This = this;
            this.city = [];
            this.city = this.cityLevel[this.$els.rovincesedit.value];
            this.provincialLevel.forEach(function(ele,index){
                if(ele.id == This.$els.rovincesedit.value){
                    This.editAddress.Province =  ele.title;
                }
            }) 
		},
		chooseProvincesAdd:function(){
			var This = this;
            this.city = [];
            this.city = this.cityLevel[this.$els.rovincesadd.value];
            this.provincialLevel.forEach(function(ele,index){
                if(ele.id == This.$els.rovincesadd.value){
                    This.editAddress.Province =  ele.title;
                }
            }) 
		},
